...
const vector<string> INPUT_FILES = {"./input1.txt", "./input2.txt"};
const vector<string> OUTPUT_FILES = {"./output1.txt", "./output2.txt"};
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < INPUT_FILES.size(); ++i) {
        LOG(INFO) << "Processing input file: " << INPUT_FILES[i];

        // Load input matrix
        auto input_matrix = read_matrix_from_txt(INPUT_FILES[i]);
        ...
        // Save final output matrix to corresponding file
        write_matrix_to_txt(OUTPUT_FILES[i], output_matrix);

        LOG(INFO) << "Finished processing input file: " << INPUT_FILES[i];
    }

    LOG(INFO) << "Inference completed for all inputs.";
    return 0;
}